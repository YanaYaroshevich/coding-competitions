...
for (let rockI = 0; rockI < movesCount; rockI++) {
  ...
  if (rockI % figures.length === 0) {
    ...
    if (memory[memoryKey] && !flag) {
      console.log(rockI);
      const mod =
        (movesCount - memory[memoryKey].rockI) %
        (rockI - memory[memoryKey].rockI);
      const dif =
        (movesCount - memory[memoryKey].rockI - mod) /
        (rockI - memory[memoryKey].rockI);

      highestPoint =
        memory[memoryKey].highestPoint +
  ...
  while (true) {
    ...
    if (!canMoveVer) {
      let tmp = Math.max(...[caveHeight, cave.length - curPosition.y]);
